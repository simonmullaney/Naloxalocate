...
class FlaskrTestCase(unittest.TestCase):
    ...
    def test_post_get_delete_users(self):
        # Create user 1
        rv = self.app.post('/naloxalocate/api/v1.0/users')
        ...
        # Get all users
        rv = self.app.get('/naloxalocate/api/v1.0/users')
        assert '{"users":[[1,null,null],[2,null,null]]}' in rv.data

        # Get user 2 only
        rv = self.app.get('/naloxalocate/api/v1.0/users/2')
        assert '{"user":[[2,null,null]]}' in rv.data

        # Get non existing user
        rv = self.app.delete('/naloxalocate/api/v1.0/users/3')
        assert response404 in rv.data

        # Delete user 2
        rv = self.app.delete('/naloxalocate/api/v1.0/users/2')
        ...
        # Delete user 1
        rv = self.app.delete('/naloxalocate/api/v1.0/users/1')
        assert '{"result":true}' in rv.data
